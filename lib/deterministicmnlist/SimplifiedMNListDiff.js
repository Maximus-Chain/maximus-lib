  ...
  for (let i = 0; i < mnListSize; i += 1) {
    ...
    if (data.nVersion >= 2) {
      ...
      if (type === constants.MASTERNODE_TYPE_HP) {
        entryBufferParts.push(
          bufferReader.read(constants.SML_ENTRY_TYPE_2_ADDITION_SIZE)
        );
      }
    }

    data.mnList.push(
      SimplifiedMNListEntry.fromBuffer(Buffer.concat(entryBufferParts), network)
    );
  }
